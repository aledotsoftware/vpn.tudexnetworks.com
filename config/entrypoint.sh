...
audit_log() {
    ...
    # Send to MariaDB (memory requirement)
    if [ -n "$DB_HOST" ] && [ -n "$DB_USER" ] && [ -n "$DB_NAME" ]; then
        # Load db password from secret if missing
        if [ -z "$DB_PASS" ] && [ -f /run/secrets/db_pass ]; then
        ...
        mariadb -h "$DB_HOST" -u "$DB_USER" "$DB_NAME" -e "INSERT INTO security_audit (event_type, description, ip_source) VALUES ('$event_type', '$escaped_desc', '$ip_source');" 2>/dev/null || true
    fi
}
