...
audit_log() {
    ...
    if command -v mariadb >/dev/null 2>&1; then
        ...
        SAFE_TIME=$(printf "%s" "$timestamp" | sed 's/\\/\\\\/g; s/'\''/''/g')
        mariadb -h "$DB_HOST" -u "$DB_USER" "$DB_NAME" -e "INSERT INTO security_audit (event_type, description, ip_source, created_at) VALUES ('$SAFE_EVENT', '$SAFE_DESC', '$SAFE_IP', '$SAFE_TIME');" 2>/dev/null || true
    fi
}
