...
(
    while true; do
        ...
        if ! curl -s --max-time 2 "${FIREBASE_BASE_URL}/.json?shallow=true" >/dev/null; then
            echo "[$(date -u)] SECURITY_AUDIT - EVENT: DB_ERROR - Base de datos inalcanzable durante volcado de métricas" >> /var/log/headscale_security_audit.log
        fi
