...
audit_log() {
    ...
    # Dual-logging: Firebase
    firebase_post "security_audit" "{\"event_type\":\"${event_type}\",\"description\":\"${description}\",\"ip_source\":\"${ip_source}\",\"created_at\":\"${timestamp}\"}" || true

    # Dual-logging: Legacy MariaDB with local fallback
    if [ -n "$DB_HOST" ] && [ -n "$DB_USER" ] && [ -n "$DB_NAME" ]; then
        # shellcheck disable=SC3043
        local safe_desc safe_ip
        # Sanitize single quotes to prevent SQL injection or breakage
        ...
        if timeout 2 mariadb-admin ping -h "$DB_HOST" -u "$DB_USER" --silent >/dev/null 2>&1; then
            mariadb -h "$DB_HOST" -u "$DB_USER" "$DB_NAME" -e "INSERT INTO security_audit (event_type, description, ip_source) VALUES ('${event_type}', '${safe_desc}', '${safe_ip}');" >/dev/null 2>&1 || \
            echo "[${timestamp}] DB_ERROR - Fallo al insertar: ${event_type} - ${description} - ${ip_source}" >> /var/log/headscale_security_audit.log
        else
            echo "[${timestamp}] DB_ERROR - Base de datos inalcanzable. Log local: ${event_type} - ${description} - ${ip_source}" >> /var/log/headscale_security_audit.log
        fi
    fi
}
