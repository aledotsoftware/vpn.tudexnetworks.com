...
audit_log() {
    ...
    # MariaDB (Legacy dual audit as requested)
    if [ -n "$DB_HOST" ] && [ -n "$DB_USER" ] && [ -n "$DB_NAME" ]; then
        ...
        # shellcheck disable=SC3043
        local safe_ip
        safe_event=$(printf "%s" "$event_type" | sed 's/\\/\\\\/g; s/'\''/''/g')
        safe_desc=$(printf "%s" "$description" | sed 's/\\/\\\\/g; s/'\''/''/g')
        safe_ip=$(printf "%s" "$ip_source" | sed 's/\\/\\\\/g; s/'\''/''/g')

        mariadb -h "$DB_HOST" -u "$DB_USER" "$DB_NAME" -e "INSERT INTO security_audit (event_type, description, ip_source, created_at) VALUES ('$safe_event', '$safe_desc', '$safe_ip', '$timestamp');" 2>/dev/null || true
    fi
}
...
# --- CONFIGURACIÓN DE SECRETOS MYSQL ---
...
# 1. Capa de Datos - Verificar conectividad con Firebase y MariaDB (Dual Audit)
MAX_RETRIES=15
RETRY_COUNT=0
DB_AVAILABLE=false
BACKOFF_DELAY=1
...
echo "⏳ [DB] Sincronizando con Bases de Datos (con Exponential Backoff)..."

# Validar MariaDB
if [ -n "$DB_HOST" ] && [ -n "$DB_USER" ]; then
    if ! timeout 5 mariadb-admin ping -h "$DB_HOST" -u "$DB_USER" --silent; then
        echo "[$(date -u)] SECURITY_AUDIT - EVENT: DB_ERROR - Conexión a MariaDB fallida durante inicialización" >> /var/log/headscale_security_audit.log
    fi
fi
...
# Primero intentamos cargar desde Docker Secrets
...
if [ -f "/run/secrets/headscale_noise_key" ]; then
    NOISE_KEY=$(cat /run/secrets/headscale_noise_key)
elif [ -n "$HEADSCALE_NOISE_KEY" ]; then
    NOISE_KEY="$HEADSCALE_NOISE_KEY"
else
...
if [ -n "$PRIVATE_KEY" ] && [ -n "$NOISE_KEY" ]; then
    echo "✅ [AUTH] Identidad recuperada y asegurada (Secrets/Firebase)."
...
while [ $HS_RETRIES -lt $HS_MAX_RETRIES ]; do
  ...
  if ! kill -0 $HS_PID 2>/dev/null; then
    ...
    tail -n 20 /var/log/headscale.log 2>/dev/null
